...
class SSHSystemRepository(SystemRepository):
    ...
    def get_system_info(self) -> SystemInfo:
        ...
        hostname = (
            hostname_result.stdout.strip() if hostname_result.success else "unknown"
        )

        # Get CPU temperature
        temp_result = self._client.execute_command("vcgencmd measure_temp")
        temp_str = temp_result.stdout.strip() if temp_result.success else "temp=0.0'C"
        temp_match = re.search(r"temp=(\d+\.\d+)'C", temp_str)
        cpu_temperature = float(temp_match.group(1)) if temp_match else 0.0
