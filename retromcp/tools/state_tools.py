...
class StateTools(BaseTool):
    ...
    async def handle_tool_call(
        ...
        try:
            ...
            return self.format_error(f"Error in {name}: {str(e)}")
    ...
    async def _manage_state(self, arguments: Dict[str, Any]) -> List[TextContent]:
        ...
        if action == StateAction.UPDATE:
            ...
            if not path or value is None:
                return self.format_error("Path and value are required for update action")
    ...
    def _format_success_response(self, result) -> List[TextContent]:
        ...
        if result.action == StateAction.LOAD and result.state:
            ...
            if "cpu_temperature" in state.system:
                response_text += f"  • CPU Temperature: {state.system['cpu_temperature']}°C\n"
            ...
            if state.controllers:
                for controller in state.controllers:
                    status = "✅ Configured" if controller.get("configured") else "❌ Not configured"
            ...
            diff = result.diff
            has_changes = bool(diff.get("added") or diff.get("changed") or diff.get("removed"))
            
            if not has_changes:
                response_text += "🎉 No differences found - system state is up to date!\n"
            else:
                ...
                if diff.get("changed"):
                    ...
                    for path, change in diff["changed"].items():
                        response_text += f"  • {path}: {change['old']} → {change['new']}\n"
                    response_text += "\n"
