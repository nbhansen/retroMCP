...
from retromcp.infrastructure.security_validator import SecurityValidator, ValidationResult
...
class TestSecurityValidator:
    ...
    def test_validate_command_blocks_dangerous_commands(self):
        ...
        for command in dangerous_commands:
            ...
            assert "blocked" in result.error_value.lower() or "dangerous" in result.error_value.lower()
    ...
    def test_validate_command_allows_safe_parameters(self):
        ...
        for command in safe_parameterized_commands:
            result = self.validator.validate_command(command)
            assert result.is_success(), f"Safe parameterized command should be allowed: {command}"
    ...
    def test_validate_path_blocks_traversal_attempts(self):
        ...
        for path in traversal_attempts:
            ...
            assert "traversal" in result.error_value.lower() or "blocked" in result.error_value.lower()
    ...
    def test_validate_path_handles_symlink_attacks(self):
        ...
        assert "symlink" in result.error_value.lower() or "resolve" in result.error_value.lower()
    ...
    def test_whitelist_is_comprehensive(self):
        ...
        legitimate_commands = [
            # System info
            "hostname", "uptime", "free", "df", "du", "ps", "top",
            # File operations
            ...
            # RetroPie specific
            "vcgencmd", "/opt/retropie", "emulationstation",
            # Network tools (safe subset)
            "ping -c", "curl -s", "wget -q"
        ]
        ...
        for base_command in legitimate_commands:
            ...
            assert result.is_success(), f"Legitimate command should be whitelisted: {base_command}"
    ...
    def test_security_validator_integration_with_existing_code(self):
        ...
        for command in current_validation_examples:
            result = self.validator.validate_command(command)
            assert result.is_success(), f"Currently accepted command should still work: {command}"